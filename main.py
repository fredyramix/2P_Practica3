...
def main():
    ...
    while c!=int(num_gen):
        ...
        #Siguiente paso selección por torneo..
        seleccionados=Torneo(funcion_penalizada)
        Imprimir (seleccionados)
        #Seleccionar el mejor de todos :D
        best666=SeleccionMejor(seleccionados,best)
        best=best666[:]
        ImprimirBest(best)
        ...
        mutados=Mutacion(cruzados,num_cromo,n)
        print "=======Mutacion============="
        Imprimir(mutados)
        primera=mutados[:]
        c=c+1
    ...
    print "N°  \tPeso   \tImportancia\tExistencia"
    ...
    for x in range(0,len(best)-1):
        try:
            if best[x][0]==1:
                ...
                impo = articulo[1]
                ex= articulo[2]
                print ""+str(x+1)+"\t\t"+str(pes)+"\t\t\t"+str(impo)+"\t\t\t"+str(ex)
            else:
                pass
        except TypeError,e:
            pass #llego a fin
